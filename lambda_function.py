...
def validate_data(age, investment_amount):
    ...
    if age is not None:
        ...
        if age < 0:
            return build_validation_result(False, "age", "The age cannot be negative.")
            
        elif age > 65:
            return build_validation_result(False, "age", "The age cannot be higher than 65.")
...
def recommend_portfolio(intent_request):
    ...
    initial_recommendation = (((risk_level.lower() == "none") * "100 % bonds (AGG), 0 % equities (SPY)") + ((risk_level.lower() == "low") * "60% bonds, 40% equities")
                                + ((risk_level.lower() == 'medium') * "40% bonds, 60% equities") + ((risk_level.lower() == 'high') * "20% bonds, 80% equities"))
                                
    return close(intent_request['sessionAttributes'], "Fulfilled", 
            {
                'contentType': "PlainText",
                'content' : f"Thank you for your information. Based on your risk level, my recommendation is to allocate your funds: {initial_recommendation}"
            })
